...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val port = 8191
    val path = "/test"
    val myServer = new NodeScala.Default(port)
    val myServerSubscription = myServer.start(path)(request => (request.values reduce (_++_)).iterator)
    println(s"Access me at localhost:$port$path")

    // TO IMPLEMENT
    // 2. create a future that expects some user input `x`
    //    and continues with a `"You entered... " + x` message
    val userInterrupted: Future[String] = Future.userInput("") map (s => "You entered... " + s)
